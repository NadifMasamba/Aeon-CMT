...
async def stats(_, message):
    ...
    system_info = f'<b>{quote}</b>\n\n'\
        f'Bot uptime:<code> {currentTime}</code>\n'\
        f'Sys uptime:<code> {osUptime}</code>\n'\
        f'CPU usage:<code> {cpuUsage}%</code>\n'\
        f'RAM usage:<code> {memory.percent}%</code>\n'\
        f'Disk usage:<code> {disk}%</code>\n'\
        f'Free space:<code> {get_readable_file_size(free)}</code>\n'\
        f'Total space:<code> {get_readable_file_size(total)}</code>\n\n'
